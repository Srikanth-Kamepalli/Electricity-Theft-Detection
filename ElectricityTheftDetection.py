...
def sendGmail():
    with open('C:\\Users\\yours\\Desktop\\ETD\\Dataset\\ET.csv', 'r') as csv_file:
        csv_reader = csv.reader(csv_file)
        
        for row in csv_reader:
            # Access the data in the second column (index 1)
            column_data = row[5]
            print(column_data)

            # Perform your desired actions based on the column data
            if column_data == '0':
                # Do something when the column data is 'some_value'
                print("No Theft")
            elif(column_data== '1'):
                # Do something else when the column data doesn't match 'some_value'
                print("Theft Detected")
                yag = yagmail.SMTP("autoemailsender2@gmail.com", "tczewxnxfrpviped")
                yag.send(to='srikanthkamepalli22@gmail.com', subject="Alert", contents=f'Theft Detected in district {row[0]} with client_id {row[1]} in the region {row[3]}')
                yag.close()
...
sendgmail = Button(main, text="Send Gmail", command=sendGmail)
sendgmail.place(x=800,y=150)
sendgmail.config(font=font1)
